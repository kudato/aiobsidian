...
- **Breaking**: one name per operation, whichever transport you are holding. REST `vault.get()`, `vault.update()`, `active.get()` and `active.update()` are `read()` and `write()`, the spelling the CLI already used, and CLI `vault.create()` is `write()` as well. `write()` means create-or-replace on both, so the CLI's `overwrite` defaults to `True`; `overwrite=False` still refuses a file that exists, which is the one thing REST cannot do
