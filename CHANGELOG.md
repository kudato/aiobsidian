...
- **Breaking**: a malformed success answer raises `APIParseError` instead of leaking past the hierarchy — `commands.list()` on a body without its envelope raised a bare `KeyError`, and the search methods and the model-returning reads raised pydantic's `ValidationError`. Code that caught those has to catch `APIParseError`, or `APIError` above it
